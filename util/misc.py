...
#     output_dir = Path(args.output_dir)
#     if best_model:
#         checkpoint_path = output_dir / 'best_model.pth'  # 固定文件名为best_model.pth
#     else:
#         epoch_name = str(epoch)
...
def all_reduce_mean(x):
    world_size = get_world_size()  # 假设这个函数正确地返回了全局进程数
    if world_size > 1:
        # 如果x不是张量，首先将其转换为张量
        if not torch.is_tensor(x):
            x = torch.tensor(x)
        # 将x的副本移到合适的设备上，同时确保它不跟踪梯度
        x_reduce = x.clone().detach().to('cuda')  # 假设我们使用的是CUDA设备
        dist.all_reduce(x_reduce)
        x_reduce /= world_size
        return x_reduce.item()
    else:
